...
class PaymentDao extends MySqlDao
{
	...
	public function get_list(array $arr_where, $offset=0, $limit=100, array $arr_sort)
	{
		try
		{
			...
			$sort   = isset($arr_sort['sort'])? $arr_sort['sort']: 'date';
			...
			$arr_columns = array(
				...
				array('p.payment_reserve_id', 'payment_reserve_id'),
				array('p.payment_reserve_status', 'payment_reserve_status'),
				array('pr.date_from', 'date_from'),
				array('pr.date_to', 'date_to'),
				array('pr.every_type', 'every_type'),
				array('pr.every_month_selected', 'every_month_selected'),
				array('pr.every_day_selected', 'every_day_selected'),
				array('pr.every_dayofweek_selected', 'every_dayofweek_selected'),
				array('p.created_at', 'created_at'),
				array('p.updated_at', 'updated_at'),
			);
			$query = \DB::select_array($arr_columns);
			$query->from(array('payment', 'p'));
			$query->join(array('fix', 'f'), 'left');
			$query->on('p.fix_id', '=', 'f.id');
			$query->on('f.is_deleted', '=', \DB::expr('0'));
			$query->join(array('payment_reserve', 'pr'), 'left');
			$query->on('p.payment_reserve_id', '=', 'pr.id');
			$query->on('pr.is_deleted', '=', \DB::expr('0'));
	...
	public function reserve_update($arr_values, $id)
	{
		$query = \DB::update('payment');
		$query->set($arr_values);
		$query->where('id', '=', $id);
		$query->where('payment_reserve_status', '=', '0');
		$query->where('is_deleted', '=', '0');
		return $query->execute();
	}

	public function update_by_reserve_id($arr_values, $reserve_id, $date_from=null, $date_to=null)
	{
		$query = \DB::update('payment');
		$query->set($arr_values);
		if ( ! empty($date_from))
		...
		$query->where('payment_reserve_id', '=', $reserve_id);
		$query->where('payment_reserve_status', '=', '0');
		$query->where('is_deleted', '=', '0');
		return $query->execute();
	}
	...
	{
		$query = \DB::delete('payment');
		$query->where('id', '=', $id);
		return $query->execute();
	}

	public function reserve_delete($payment_reserve_id, $date_from=null, $date_to=null)
	{
		$query = \DB::delete('payment');
		if ( ! empty($date_from))
		{
		...
		$query->where('payment_reserve_id', '=', $payment_reserve_id);
		$query->where('payment_reserve_status', '=', '0');
		$query->where('is_deleted', '=', '0');
		return $query->execute();
	}
